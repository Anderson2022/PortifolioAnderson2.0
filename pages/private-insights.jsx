  ...
  return (
    <div className="min-h-screen bg-base px-5 py-10 font-body text-ink md:px-10">
      ...
      <main className="mx-auto max-w-6xl">
        ...
        <section className="neu mt-6 overflow-x-auto p-6"><h2 className="font-display text-lg font-bold">Acessos recentes</h2><table className="mt-5 w-full min-w-[900px] text-left font-mono text-xs"><thead className="text-faint"><tr><th className="pb-3">Data</th><th>Página</th><th>Local</th><th>Origem</th><th>Campanha</th><th>Duração</th></tr></thead><tbody>{filteredSessions.slice(0, 30).map((item) => <tr key={item.id} className="border-t border-deepest text-muted"><td className="py-3">{item.startedAt ? new Date(item.startedAt).toLocaleString('pt-BR') : '—'}</td><td>{item.lastPage}</td><td>{item.city} / {item.country}</td><td>{item.referrerUrl ? <a href={item.referrerUrl} target="_blank" rel="noreferrer" className="block max-w-[220px] truncate text-plum hover:underline" title={item.referrerUrl}>{item.referrerUrl}</a> : item.source}</td><td>{item.utmSource ? `${item.utmSource}${item.utmCampaign ? ` / ${item.utmCampaign}` : ''}` : '—'}</td><td>{item.durationSec || 0}s</td></tr>)}</tbody></table></section>
      </main>
    </div>
  );
}
